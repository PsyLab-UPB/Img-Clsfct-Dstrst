/**
 * @title FormsOnline_Classification_Session1
 * @description Session 1 of FormsClassification-1 Online-version
 * @version 1.0.0
 *
 * @assets assets/
 * @imageDir images
...
export async function run({ assetPaths, input = {}, environment, title, version }) {
  ...
  timeline.push({
    ...
    inclusion_function: (data) => {
      return data.width >= 1600 && data.height >=920
      },
    allow_window_resize: true,
    minimum_width: 1600,
    minimum_height: 920,
    exclusion_message:  (data) => { 
      if(data.height < 921 || data.width < 1601) {
  ...
  timeline.push({
    ...
    stimulus: () => {
      return `<iframe class="declaration" src="assets/misc/declaration_de_Prolific_Forms1.html"></iframe>`;
    },
  ...
  timeline.push(
    {
      type: HtmlButtonResponsePlugin,
      stimulus: `Bitte stellen Sie sicher, dass der Ton eingeschaltet ist. <br>`,
      choices: ["Ton ist eingeschaltet."]
    }
  );
    ...
    timeline: [
      {
        ...
        questions: [{ prompt: "Bitte geben Sie Ihre Prolific-ID an:", required: true }],
        button_label: "Weiter"
      },
      {
        type: SurveyTextPlugin,
  ...
  <p>Sie haben das Experiment abgeschlossen. Klicken Sie auf 'Weiter', um das Experiment abzuschließen und zu Prolific zurückzukehren.</p>
  `
